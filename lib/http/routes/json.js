...
const deleteJobsFromIds = (ids, state) => {
  if (!ids) return false

  ids.forEach(id => {
    Queue.Job.get(id, (err, job) => {
      if (err) return

      if (state === job._state) job.remove()
    })
  })
}
...
/**
 * Search jobs by their state and a given search query.
 */
exports.jobByStateSearch = (req, res) => {
  var state = req.params.state
    , from  = parseInt(req.params.from, 10)
    , to    = parseInt(req.params.to, 10)
  ...
  getSearch().query(query).end((err, searchIds) => {
    if (err) return res.json(err)
    queue[state]((err, stateIds) => {
      if (err) return res.json(err)
      const returnedJobs = []
      const filteredIds = searchIds
        .filter(id => ~stateIds.indexOf(parseInt(id)))

      if(filteredIds.length === 0) return res.json(filteredIds)
      ...
      filteredIds
        ...
        .forEach((id, index, array) => {
          Queue.Job.get(id, (err, job) => {
            ...
            if (returnedJobs.length === array.length)
              return res.json(returnedJobs)
          })
        })
    })
  })
...
/**
 * Search and delete jobs given a query and a state
 */
exports.searchAndDelete = (req, res) => {
